...
export default {
  data() {
    return {
      ...
      pkTemp: '12f3c031-4a21-4f50-8797-de42d5b251d2', // 客户列表_小贷
      currentValue: '',
      isHide: true,
      searchStr: {
        name: ''
  ...
  methods: {
    ...
    addEnterprise(type) {
      if(!type) {
        ...
        this.$router.push('/pettyloanCrm/personalInfo/add');
      } else if(type === 2) {
        this.$router.push('/pettyloanCrm/enterpriseInfo/add');
      }
    },
    //点击查询按钮触发
    search() {
    ...
    editTableRow(row) {
      if(row.type === 1 || row.type === "1") { // 企业客户
        this.$router.push('/pettyloanCrm/enterpriseInfo/' + row.id);
      } else if(row.type === 2 || row.type === "2") { // 个人客户
        this.$router.push('/pettyloanCrm/personalInfo/' + row.id);
      } else {
...
<style>
  #pettyloanCustomerList {
    padding: 20px 30px;
    margin-bottom: 30px;
  }
  /*#pettyloanCustomerList .title-header{*/
    /*height: 56px;*/
    /*padding: 10px 20px;*/
    ...
    /*margin-bottom: 6px;*/
  /*}*/
  /*#pettyloanCustomerList .title-header h2 {*/
    /*font-size: 18px;*/
    /*font-weight: bold;*/
  /*}*/
  .search-header {
  ...
  #pettyloanCustomerList .content_main {
    background-color: #ffffff;
    /*min-height: 350px;*/
    padding-bottom: 20px;
    margin-top: 20px;
  ...
  #pettyloanCustomerList .yon-uitemplate {
    margin: 20px;
  }

  /* 重置表格字体及表头背景，参考友人才 */
  #pettyloanCustomerList .el-table__header th{
    font-size: 15px;
    font-weight: 400;
    color: #333;
  }
  #pettyloanCustomerList .el-table__body .cell{
    font-size: 13px;
    font-weight: normal;
    color: #333;
  }
  #pettyloanCustomerList .el-table thead tr, #pettyloanCustomerList .el-table thead tr th {
    background-color: #eee;
  }
  #pettyloanCustomerList .el-table__footer-wrapper thead div, #pettyloanCustomerList .el-table__header-wrapper thead div {
    background-color: #eee;
    color: #333;
  }
