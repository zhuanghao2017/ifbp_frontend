<template>
<div class="mdlayout-page" id="staff-detail-page">
  <div class="mdlayout-page-header">
      ...
      <span class="header-title" data-bind="text:settlement.ref('djbm')()">员工信息-{{currentValue && currentValue.name}}</span>
      <ul class="header-btn">
        <el-tooltip content="新建员工信息" placement="top">
          <i class="header-icon el-icon-plus" @click="createStaff"></i>
        </el-tooltip>
        <el-tooltip content="编辑员工信息" placement="top">
          <i class="header-icon el-icon-pt-bianji" @click="editStaff"></i>
        </el-tooltip>
        <el-tooltip content="删除员工信息" placement="top">
          <i class="header-icon el-icon-delete" @click="deleteStaff"></i>
        </el-tooltip>
      </ul>
  </div>
...
// import uitemplate from '../../../../../../1/ifbp-element/packages/ui-template/src/template.vue';
  ...
  export default {
    ...
    mounted() {
      var thisEl = document.querySelector("#staff-detail-page");
      if(thisEl) {
        var vm = this;
        // 点击detail页面隐藏左侧列表
    ...
    methods: {
      ...
      // 点击创建工作记录为子表添加数据
      newChildItem() {
        this.$router.push("/staff/addRecord");
      },
      // 点击编辑工作记录子表某行数据
      editChildItem() {
        this.$router.push("/staff/editRecord/" + this.$parent.selectRowId);
      },
      // 点击删除工作记录子表某行数据
      deleteChildItem() {
