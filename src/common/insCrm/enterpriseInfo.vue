    ...
    <div class="detail-container">
      ...
      <div class="content-main">
        ...
        <div class="content-left">
          <!-- 基本档案开始 -->
          <div class="detail-item" id="enterprise-information" style="height: 310px;">
            <div class="detail-items" style="height:280px;">
            <div class="child-title"><b>基本信息</b><i v-if="!createFlags['enterprise-information'] && !isCreate" class="el-icon-edit" @click="toggle('enterprise-information')"></i></div>
          ...
          <!-- 深度信息开始 -->
          <div class="detail-item" id="deep-information">
            <div class="detail-items">
            <div class="child-title"><b>深度信息</b><i v-if="!createFlags['deep-information'] && !isCreate" class="el-icon-edit" @click="toggle('deep-information')"></i></div>

            <el-template :pkTemp="pktemp['deep-information']"
                          ref="deep-information"
                          :table-style="tableStyle"
                          :hide-title="true"
                          :isEdit="createFlags['deep-information']"
                          :templateValue="currentValue['deep-information']"
                          @setEmptyKeys="function(v){setEmptyKey(v,'deep-information')}"
                          @change="changeHandle"
                        >
            </el-template>
            <el-button-group class="group" v-if="createFlags['deep-information'] && !isCreate">
                <el-button class="save" type="primary" @click="submit('deep-information')">保存</el-button>
                <el-button class="save" type="primary" @click="cancel('deep-information')">取消</el-button>
            </el-button-group>
            </div>
          ...
          <!-- <div class="detail-item" id="shareCopyright-record">
            ...
            <el-row>
              <el-col class="child-title-left" :span="12">
                股东版权信息<i class="el-icon-plus" @click="addListItem('shareCopyright-record')"></i>
              </el-col>
              <el-col class="child-title-right" :span="12">
          ...
          <!-- 业务员信息开始 -->
          <div class="detail-item" id="salesman-information" style="min-height: 100px;">
            <div class="detail-items">
            <div class="child-title"><b>业务员信息</b><i v-if="!createFlags['salesman-information'] && !isCreate" class="el-icon-edit" @click="toggle('salesman-information')"></i></div>

            <el-template :pkTemp="pktemp['salesman-information']"
                          ref="salesman-information"
                          :table-style="tableStyle"
                          :hide-title="true"
                          :isEdit="createFlags['salesman-information']"
                          :templateValue="currentValue['salesman-information']"
                          @setEmptyKeys="function(v){setEmptyKey(v,'salesman-information')}"
                          @change="changeHandle"
                        >
            </el-template>
            <el-button-group class="group" v-if="createFlags['salesman-information'] && !isCreate">
                <el-button class="save" type="primary" @click="submit('salesman-information')">保存</el-button>
                <el-button class="save" type="primary" @click="cancel('salesman-information')">取消</el-button>
            </el-button-group>
            </div>
          ...
          <!-- 其他信息 -->
          <div class="detail-item" id="other-information" style="min-height: 100px;margin-bottom: 20px;">
            <div class="detail-items" style="border-bottom: 0;">
            <div class="child-title"><b>其他信息</b><i v-if="false" class="el-icon-edit" @click="toggle('other-information')"></i></div>

            <el-template :pkTemp="pktemp['other-information']"
                          ref="other-information"
                          :table-style="tableStyle"
                          :hide-title="true"
                          :isEdit="false"
                          :templateValue="currentValue['other-information']"
                          @setEmptyKeys="function(v){setEmptyKey(v,'other-information')}"
                          @change="changeHandle"
                        >
            </el-template>
            <!-- <el-button-group class="group" v-if="createFlags['other-information'] && !isCreate">
                <el-button class="save" type="primary" @click="submit('other-information')">保存</el-button>
                <el-button class="save" type="primary" @click="cancel('other-information')">取消</el-button>
            </el-button-group> -->
            </div>
    ...
    data(){
      ...
      return {
        ...
        createFlags: {
          ...
          'deep-information': isCreate,
          // 'contracts-record': isCreate,
          'bank-record': isCreate,
          'other-information': isCreate,
          'salesman-information': isCreate,
        ...
        isCreate: isCreate,
        formForCreate: ['enterprise-information','deep-information','salesman-information'],
        isEdit: {
          'enterprise-information': isCreate,
          'deep-information': isCreate,
          // 'contracts-record': isCreate,
        ...
        pktemp: {
          'enterprise-information': '6decb351-7765-449e-b258-56524abbf66b', // 企业用户基本信息_保险
          'deep-information': '35447627-f329-4d10-be48-3e96cb3ac503', // 深度信息
          // 'contracts-record': 'bb6821f4-572c-427b-b81b-59ccbdb7d898',
          'bank-record-form': '628384b7-fa91-4c59-9944-6865fe78ae76', // 企业客户银行账户表单_保险
          'bank-record':'2ffe8af6-d0a0-49f4-a224-2e45291be977', // 企业客户银行账户表格_保险
          'salesman-information': 'ebcb7181-5b24-4d57-85b2-a94e62dbac94', // 业务员信息
          'other-information': '1540f9cf-1d67-4961-bdf7-40d69bda8191' // 其他信息
        },
        promises: {},
        expands: {},
        // 存储ui模板grid展开列
      ...
      idMap:{
        ...
        // 'salesman-information': 'id_entshareholder'
      },
      idMapChild:{
        // 'contracts-record': 'fk_id_entperson',
        'bank-record': 'id_entbank',
        // 'salesman-information': 'fk_id_entshareholder'
      },
      tableStyle: {
        'max-height': '1000px'
      },
    ...
    requestBank() { // 获取银行账户值数据
      ...
      assignValues(id){
        if (id === 'bank-record') { // handle individual bank component
          ...
          // 修复参照初次加载不显示的问题
          this.currentValue[id].beanMap = this.templateValue.beanMap || {};
        }
      ...
      createForm(){
        this.handleMultiValidate(['enterprise-information','deep-information']).then(() => {
          var submitData = {id: ''};
          ...
          this.$http({
            ...
            if(res.data.success === true) {
              ...
              this.$router.push('/insCrm/enterpriseInfo/' + this.id);
     ...
     //设置空值
      ...
      changeHandle(data) {
        ...
        if(data.key === 'salesman') {
          this.$set(this.currentValue['salesman-information'], 'def1', data.value && data.value.length ? data.value[0].refcode : '');
        }
      },
      backButton() {
      ...
      scroll() {
        if(this.$route.path.indexOf('/insCrm/enterpriseInfo') < 0){
          return;
        }
        this.fixedNav();
        this.scrollFocus();
      ...
      // 控制side-nav悬浮在页面右上方
      fixedNav() {
        if($('#ins-enterprise-info .content-main').length > 0) {
          const mainTop = $('#ins-enterprise-info .content-main').offset().top;
          const $fixedDom = $('#ins-enterprise-info .content-right');
          if(mainTop >= 82) {
            ...
            if(valid) {
              ...
              if(!this.isAddBankForm){
                ...
                self.$http({
                  ...
                  if(res.data.success === true) {
                    self.currentValue['bank-record'].push(submitData);
                    self.originalValue['bank-record'].push(submitData);
                    self.$message({
                      message:'保存成功',
                    });
                    self.showBankForm = false;
...
<style>
  #ins-enterprise-info .detail-container .el-form-item {
    margin-bottom: 22px;
  }
  #ins-enterprise-info .detail-container input.el-input__inner {
    height: 28px !important;
  }
  #ins-enterprise-info .detail-container .el-upload__input {
  display: none;
  }
  #ins-enterprise-info .detail-container .el-form-item__label {
    font-size: 13px;
  }
  #ins-enterprise-info .detail-container .save {
    width: 100px;
    margin-left: calc(50% - 50px);
  }
  #ins-enterprise-info .detail-container .group .save {
    width: 100px;
    margin-left: 10px;
    border-radius: 4px;
  }
  #ins-enterprise-info .detail-container .group {
    width: 220px;
    margin-left: calc( 50% - 110px);
    margin-bottom: 20px;
  }
  #ins-enterprise-info .detail-container .el-upload.el-upload--text {
    height: 100%;
    width: 100%;
  }
  #ins-enterprise-info .detail-container .avatar-uploader .el-upload:hover {
    border-color: #20a0ff;
  }
  #ins-enterprise-info .detail-container .avatar-uploader .el-upload {
...
/*  #ins-enterprise-info .detail-container .el-table td.is-left, .el-table th.is-left{
    text-align: center;
  }*/
  #ins-enterprise-info .detail-container .el-table .cell{
    padding-left:10px;
    padding-right:0;
  }
</style>
...
/*    #ins-enterprise-info{
  ...
  #ins-enterprise-info .content-left{
    position:relative;
    width: calc(100% - 175px);
    float: left;
    margin-left:0;
  }
  #ins-enterprise-info .content-right {
    width: 164px;
    float: right;
    position: relative;
  ...
  #ins-enterprise-info .content-right.fixed {
    width: 164px;
    position: fixed;
    right: 20px;
    top: 80px;
