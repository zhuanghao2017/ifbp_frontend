...
Vue.use(Vuex);
/* 因为单独引入vue的话会报错误但并不影响使用 [vuex] already installed. Vue.use(Vuex) should be called only once */
...
for(const item in state1){
 if (localStorage.getItem(item)) {
   state1[item] = JSON.parse(localStorage.getItem(item));
 }
}
...
const mutations = {
    ...
    setAppState(state, payload) {
        Object.assign(state.appState, payload);
