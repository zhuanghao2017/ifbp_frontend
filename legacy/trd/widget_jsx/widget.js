var WidgetEditTool = React.createClass({
	...
	render: function(){
		return (
			<ul className="list-unstyled edit">
				<li><i className="portalfont btn btn-round btn-default btn-outline btn-pill-right icon-max" data-type="window" title="最大最小化"></i></li>
				<li><i className="portalfont btn btn-default btn-outline icon-unfold" data-type="collage" title="折叠"></i></li>
				<li><i className="portalfont btn btn-round btn-default btn-outline btn-pill-left icon-pencil" data-type="edit" data-toggle="modal" data-target="#modalBlue" title="编辑"></i></li>
				<li><i className="portalfont btn btn-default btn-outline icon-cancel02" data-type="del" title="删除"></i></li>
			</ul>
		)
	}
})
...
var Widget = React.createClass({
	...
	setWidgetEditTool:function(){
		return this.props.layoutContext.options.isWidgetEdit?<WidgetEditTool />:'';
	},
	...
	render : function() {
		...
		return (
			<div>
				<div id={'gadget-site-'+this.props.id} data-id={this.props.id} data-ul={this.props.gadgetURL} className={this.props.layoutContext.options.isSortable?'well sort':'unsortable'}>
					{ this.props.layoutContext.options.isShowWidgetName?<h5>{widgetMeta.modulePrefs?widgetMeta.modulePrefs.title:'modulePrefs is undefined'}</h5>:''}
					<div id={'gadget-site-content-'+this.props.id} className={isUrl?'innerHtml-ipad collapse in':'innerHtml collapse in'} dangerouslySetInnerHTML={{__html:this.state.html}}>
					</div>
...
var WidgetBox = React.createClass({
    ...
    render: function () {
        ...
        return (
            <div id={this.props.id} key={this.props.id} className="widgetBox connectedSortable">
                {
                    React.Children.map(
                        this.props.children,
                        function (result) {
                            if (result.type.displayName && result.type.displayName === "Widget") {
                                return (
                                    <Widget id={result.props.id} key={result.props.id}
                                            ...
                                            widgetBoxId={widgetBoxId}></Widget>
                                )
                            }
                        }
                    )
